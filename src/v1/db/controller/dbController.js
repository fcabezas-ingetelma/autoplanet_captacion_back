        ...
        if(tokenValidation.length && tokenValidation[0].token === token) {
            ...
            response.dailyStatus = await db.query(`SELECT V.*, Enrolado_Vendedor, Enrolado_Ipad, Enrolado_Limpia_Tapiz  FROM
                                                    ...
                                                    SUM(case when canal=19 then 1 ELSE 0 END) Visita_Limpia_Tapiz
                                                        FROM (SELECT IP, canal, MIN(created_at) AS created_at from tracker WHERE canal IN (2,18,19) AND Month(DATE_ADD(created_at, INTERVAL -3 HOUR)) =month(DATE_ADD(NOW(), INTERVAL -3 HOUR)) AND HOUR(created_at)>=11 GROUP BY IP, Canal) AS A
                                                    ...
                                                    SUM(case when canal=19 then 1 ELSE 0 END)
                                                        FROM (SELECT IP, canal, MIN(created_at) AS created_at from tracker WHERE canal IN (2,18,19) AND Month(DATE_ADD(created_at, INTERVAL -3 HOUR)) =month(DATE_ADD(NOW(), INTERVAL -3 HOUR)) AND HOUR(created_at)>=11 GROUP BY IP, Canal)  AS A WHERE canal IN (2,18,19)
                                                ...
                                                LEFT Join
                                                    ...
                                                    AND Month(DATE_ADD(created_at, INTERVAL -3 HOUR)) =month(DATE_ADD(NOW(), INTERVAL -3 HOUR))AND HOUR(created_at)>=11
                                                    GROUP BY right(CONCAT('0',cast(day(DATE_ADD(created_at, INTERVAL -3 HOUR)) AS CHAR)),2)
                                                    UNION all
                                                    SELECT 'Total',
                                                    SUM(case when canal=2 then 1 ELSE 0 END) ,
